...
public class OldAreasMigration {
    ...
    // TODO
    public void migrate() {
        if (DiscoverAreasPlugin.getConfiguration().contains("areas")) {
            // Let's migrate
            ...
            DiscoverAreasPlugin.getConfiguration().getConfigurationSection("areas").getKeys(false).forEach(name -> {
                ...
                try {
                    ...
                    Area area = new Area(id, region, world, name, description, xp, sound, material, commands, created);
                    area.addToCache();
